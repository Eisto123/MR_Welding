...
namespace TryAR.MarkerTracking
{
    ...
    public class ArUcoMarkerTracking : MonoBehaviour
    {
        ...
        /// </summary>
        private Dictionary<int, OpenCVARUtils.PoseData> _prevPoseDataDictionary = new Dictionary<int, OpenCVARUtils.PoseData>();
        ...
        public void DetectMarker(Texture webCamTexture, Texture2D resultTexture = null)
        {
            if (_isReady)
            {
                ...
                Texture2D processedTexture = webCamTexture as Texture2D;
                
                // Get image from webcam at full size
                OpenCVMatUtils.Texture2DToMat(processedTexture, _originalWebcamMat);
            ...
            ))
            {
                ...
                for (int i = 0; i < _detectedMarkerCorners.Count; i++)
                {
                    ...
                    using (MatOfPoint2f imagePoints = new MatOfPoint2f(corner_4x1))
                    {
                        ...
                        OpenCVARUtils.PoseData poseData = OpenCVARUtils.ConvertRvecTvecToPoseData(rvecArr, tvecArr);

                        // Get previous pose for this marker (or create new)
                        if (!_prevPoseDataDictionary.TryGetValue(currentMarkerId, out OpenCVARUtils.PoseData prevPose))
                        {
                        ...
                        Debug.Log($"Marker ID: {currentMarkerId}");
                        Debug.Log($"Position: {poseData.Pos}");
                        Debug.Log($"Rotation: {poseData.Rot.eulerAngles}");
